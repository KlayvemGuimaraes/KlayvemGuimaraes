# Klayvem Guimarães | AI Engineer & Software Engineer
[![Available Now](https://img.shields.io/badge/AVAILABLE-NOW-27ae60?style=for-the-badge)]()
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-020114?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/klayvemguimaraes/)
[![Currículo PDF](https://img.shields.io/badge/-Currículo-020114?style=for-the-badge&logo=adobe-acrobat-reader)]([Klayvem_Guimarães_cv_pt.pdf](https://github.com/user-attachments/files/21701270/Klayvem_Guimaraes_cv_pt.pdf))
[![Portfolio](https://img.shields.io/badge/-Portfolio-020114?style=for-the-badge&logo=)](https://klayvemguimaraes.netlify.app/)
[![GitHub Projects](https://img.shields.io/badge/-Projetos-020114?style=for-the-badge&logo=github)](https://github.com/klayvemguimaraes?tab=repositories)

<div align="center">
  <img src="https://github.com/user-attachments/assets/3e541d80-8aa8-4432-be39-8ab0c9a5f260" alt="Tech Banner" width="100%">
</div>

## 🔍 Sobre Mim
- **Engenheiro de Software** com foco em back-end e soluções de IA generativa
- **Experiência** com TypeScript, NestJS, Python, Java e arquitetura de sistemas:
- **3+ anos** construindo sistemas fullstack
- **Cursando Bacharelhado em Engenharia da Computação** pela [Universidade Virtual do Estado de São Paulo](https://univesp.br/)
- **Objetivo**: Desenvolver soluções inovadoras que integram GenAI com sistemas robustos de back-end, projetando arquitetura e entregando código limpo e útil.
- **Skills**:
  - Trabalho em equipe multidisciplinar.
  - Resolução de problemas.
  - Proatividade, comunicação e oratória.

## 🛠️ Stack Principal
[![Java](https://img.shields.io/badge/Java-020114?style=for-the-badge&logo=openjdk&logoColor=white)](https://www.java.com/)
[![Python](https://img.shields.io/badge/-Python-020114?style=for-the-badge&logo=python)](https://www.python.org/)
[![TypeScript](https://img.shields.io/badge/-TypeScript-020114?style=for-the-badge&logo=typescript)](https://www.typescriptlang.org/)
[![NestJS](https://img.shields.io/badge/-NestJS-020114?style=for-the-badge&logo=nestjs&logoColor=E0234E)](https://nestjs.com/)
[![React](https://img.shields.io/badge/-React-020114?style=for-the-badge&logo=react)](https://reactjs.org/)
[![AWS](https://img.shields.io/badge/AWS-020114?style=for-the-badge&logo=amazonwebservices&logoColor=white)](https://aws.amazon.com/)

## 💼 Experiência 

**AI Engineer @ [Rivio](https://www.rivio.com.br/)**  
_Out 2025 - Maio 2026 (9 meses) | Presencial_  

Health-tech focada em soluções de IA para operações hospitalares, faturamento e auditoria de contas médicas.

- Projetei e implantei um agente GenAI para automatizar a identificação e tratamento de bloqueios em guias médicas XML, substituindo um fluxo 100% manual.

- Reduzi em **90%** o tempo de processamento desse fluxo, aumentando eficiência, rastreabilidade e escala para uma operação com aproximadamente **50 profissionais**.

- Desenvolvi **20+ integrações e automações críticas** para o ciclo de faturamento, combinando RPA, APIs, filas assíncronas e componentes de IA.

- Reduzi processos operacionais de **6–10 minutos** para **30–60 segundos**, sustentando **100+ execuções diárias** com lógica de retry e fallback.

- Criei uma biblioteca interna de validação com `Python` e `Pydantic` para centralizar schemas XSD do padrão TISS, melhorando consistência e reutilização nos times de backend.

**Tech stack:** `Python` `FastAPI` `Vertex AI` `Gemini` `Agno` `LiteLLM` `Playwright` `Redis` `MongoDB` `Docker` `GCP` `Pub/Sub` `Pydantic`

**Software Engineer @ [DigAI](https://www.digai.ai/pt-BR)**  
_Dez 2024 - Jul 2025 (8 meses) | Remoto_  

Startup focada em soluções de IA para recrutamento, automação de cobranças e fluxos conversacionais.

- Desenvolvi automações backend com `TypeScript` e `NestJS`, reduzindo em **83%** o tempo operacional gasto em tarefas manuais.

- Projetei, testei e otimizei agentes de IA Generativa para cobranças autônomas via WhatsApp.

- Modelei fluxos conversacionais com foco em NLP, segmentação comportamental e lógicas de fallback.

- Atuei em colaboração com Produto, UX e Comercial para alinhar arquitetura, tom de voz e objetivos de conversão.

**Tech stack:** `TypeScript` `NestJS` `Node.js` `GenAI` `NLP` `WhatsApp APIs`

**Game Tester (UX e QA) @ [Spcine](https://www.circuitospcine.com.br/)**  
_Jan 2024 - Dez 2024 (1 ano) | Híbrido_  

Atuação no SpGame Lab, iniciativa voltada ao suporte técnico e à melhoria da qualidade de jogos e produtos digitais.

- Realizei testes de QA, UX e estresse em jogos e aplicações ainda em desenvolvimento.

- Identifiquei, documentei e reportei bugs funcionais, falhas técnicas e problemas de usabilidade.

- Apoiei mais de **20 estúdios e empresas parceiras** na melhoria da qualidade de seus produtos.

- Testei e reportei problemas em mais de **50 jogos e sistemas**, contribuindo para entregas mais estáveis e refinadas.

**Tech Stack:** `QA` `UX Testing` `Bug Reporting` `Functional Testing` `Game Testing`

## 🔖 Certificações
- **[AWS AI Practioner](https://www.credly.com/badges/c7832435-7fdf-4fbb-87ab-659572e8131c/public_url)**
- **[CompTIA Cloud Essentials +](https://www.credly.com/badges/d42bc635-7e1c-438f-92eb-d82cc94b86cd/public_url)**
- **[Expert Agentic Engineer L1](https://deco-certify.deco.page/certificado/60a395cd-0305-4c42-9342-ef9570d36725)**

## 🏆 Destaques
- **Top Performer** - Projeto de automação da DigAI (reconhecimento interno)
- **Campeão em 15+ Hackathons** em 100+ participações:     
   - **1º Lugar Hacking Medicine MIT & Albert Einstein**  
     Plataforma CoreCare - Triagem hospitalar assistida por AI  
     Stack: Typescript, Node, Gemini
   - **1º Lugar Edge AI Developer Qualcomm**  
     Plataforma Bemmover - Fisioterapia assistida por AI  
     Stack: Python, LLM Edge
   - **1º Lugar EmpreendeSIM USP 2023**  
     Solução de segurança para mulheres ciclistas do estado de São Paulo
     Stack: React, TypeScript

## 📬 Vamos Conversar?
[![Agendar Reunião](https://img.shields.io/badge/-Agendar_30min-020114?style=for-the-badge&logo=calendly)](https://calendly.com/klayvemguik/30min)
[![Email](https://img.shields.io/badge/-Email-020114?style=for-the-badge&logo=gmail)](mailto:klayvemguik@gmail.com)
